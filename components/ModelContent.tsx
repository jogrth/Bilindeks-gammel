...
export function ModelContent({ model }: ModelContentProps) {
  const hasSEOContent = model.seo_content && model.seo_content.sections?.length > 0;
  const hasFAQ = model.faq_content && model.faq_content.length > 0;
  ...
  return (
    <div className="space-y-8">
      <div className="bg-white rounded-xl border border-slate-200 p-8">
        ...
        {hasSEOContent ? (
          <div className="space-y-6 text-slate-700 leading-relaxed">
            {model.seo_content!.sections.map((section, idx) => (
              <div key={idx}>
                <h3 className="text-xl font-semibold text-slate-900 mt-8 mb-4 first:mt-0">
                  {section.heading}
                </h3>
                <p>{section.content}</p>
              </div>
      ...
      {hasFAQ && (
        <div className="bg-white rounded-xl border border-slate-200 p-8">
          <h2 className="text-2xl font-bold text-slate-900 mb-6">
            Ofte stilte spørsmål om {model.brand_name} {model.name}
          </h2>
          <div className="space-y-6">
            {model.faq_content!.map((faq, idx) => (
